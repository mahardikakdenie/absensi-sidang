...
watch(() => props?.projectId, () => {
	...
	timeOut.value = store?.typeAction?.data?.shift?.[0]?.shift?.timeOut ?? '';
});
...
const submit = () => {
    console.log(store?.typeAction?.data?.users);
    const userIds = store?.typeAction?.data?.users?.map(user =>  user?.user_id);
    const params = {
